...
export const login = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(401).json({ message: "Hibás elérhetőség vagy jelszó." });
    }
    ...
    if (user.allapot === "inaktiv") {
      if (user.elerhetoseg === "admin@local") {
        ...
        return res.status(403).json({
          message: "A fiók inaktív. Vedd fel a kapcsolatot az ügyfélszolgálattal.",
        });
      }
    }
...
export const getProfile = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Szerver hiba történt a profil lekérése során." });
  }
};
  ...
  try {
    ...
    if (missing.length > 0) {
      ...
      return res.status(500).json({
        message: "Google OAuth nincs beállítva a szerveren.",
        missing,
      });
    }
    ...
    res.status(500).json({ message: "Google belépés indítása sikertelen." });
  }
};
...
export const handleGoogleOAuthCallback = async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res.redirect(buildOauthErrorRedirect("Google belépés megszakítva."));
    }
    if (!code) {
      logOauth("Google", "missing auth code");
      return res.redirect(buildOauthErrorRedirect("Google kód nem érkezett meg."));
    }
    ...
    if (missing.length > 0) {
      ...
      return res.redirect(
        buildOauthErrorRedirect(
          `Google OAuth nincs beállítva a szerveren. Hiányzik: ${missing.join(", ")}`,
        ),
      );
    }
    const redirectUri = process.env.GOOGLE_REDIRECT_URI;
    ...
    if (!userInfoResponse.ok) {
      ...
      return res.redirect(
        buildOauthErrorRedirect("Google felhasználói adatok lekérése sikertelen."),
      );
    }
    ...
    res.redirect(buildOauthErrorRedirect("Google belépés sikertelen."));
  }
};
...
export const updateOwnProfile = async (req, res) => {
  try {
    ...
    if (!nev || !elerhetoseg) {
      return res.status(400).json({ message: "A név és email megadása kötelező." });
    }

    if (!USERNAME_REGEX.test(nev)) {
      return res.status(400).json({
        message:
          "A felhasználónév érvénytelen. 3-30 karakter hosszú lehet, és csak betűket (ékezeteseket is), számokat, szóközt, pontot, kötőjelet és aláhúzást tartalmazhat.",
      });
    }
    ...
    if (newPassword) {
      if (!PASSWORD_REGEX.test(newPassword)) {
        return res.status(400).json({
          message:
            "Az új jelszó legyen legalább 6 karakter hosszú, tartalmazzon betűt és számot, és ne tartalmazzon szóközt.",
        });
      }
